...
void main() {
    ...
    depth_layers[0] = baseDepth;
    active_layers = 1;

	vec4 color = texture(TranslucentSampler, texCoord);
	float depth = texture(TranslucentDepthSampler, texCoord).r;
	//This isn't a complete implementation of the fast/fancy water shader, but will still function well at high render distances
	if(color.a != 0.0) {
		if(color.a > 0.9 && color.a < 1.0) {
			baseDepth = 1.0 - (1.0 - baseDepth) * 20.0;
			if(baseDepth > color.a) color.a = baseDepth;
		}
		try_insert(color, depth);
	}
