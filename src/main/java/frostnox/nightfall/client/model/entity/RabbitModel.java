...
public class RabbitModel extends AnimatedModel<RabbitEntity> implements HeadedModel {
    ...
    @Override
    public void setupAnim(RabbitEntity entity, float limbSwing, float limbSwingAmount, float time, float netHeadYaw, float headPitch) {
        resetPose();
        antlers.visible = entity.isSpecial();
        float speed = 3.3F;
    ...
    @Override
    public void animateStun(int frame, int duration, int dir, float mag, RabbitEntity user, AnimationCalculator mCalc, Vector3f mVec, float partialTicks) {
