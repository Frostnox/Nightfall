...
public class ChiselAndHammerCrawling extends PlayerAction {
    public ChiselAndHammerCrawling(int[] duration, Properties properties) {
        super(properties, duration);
    }

    public ChiselAndHammerCrawling(Properties properties, int... duration) {
        super(properties, duration);
    }
        ...
        if(data.size() == 6) {
            ...
            switch(state) {
                case 0 -> {
                    ...
                    rightHand.rCalc.extend(-165 + pitch, -5, 20 - pitch/4);
                    rightHand.toDefaultTranslation();
                    leftArm.rCalc.extend(0, 0, 0);
                    leftHand.rCalc.extend(-175 + pitch, 10, -20 + pitch/4);
                ...
                case 1 -> {
                    ...
                    leftHand.rCalc.add(45, 0, 0, Easing.outCubic);
    ...
    public void transformOppositeLayerSingle(int state, int frame, int duration, float charge, LivingEntity user, AnimationData data) {
        switch(state) {
            case 0 -> {
                data.rCalc.addFrom(data.dRotation, 10, 10, 0);
    ...
    public float getPitch(LivingEntity user, float partial) {
        return Mth.clamp(user.getViewXRot(partial), -55, 50);
    }
