...
public class ThrowAxeTechnique extends ThrowTechnique {
    public ThrowAxeTechnique(DamageType[] damageType, int stunDuration, int[] duration, Properties properties, float throwVelocity, float curve, boolean spinning, @Nullable AttackEffect... effects) {
    ...
    public void transformModelFP(int state, int frame, int duration, float charge, LivingEntity user, AnimationData data) {
        ...
        switch(state) {
            case 0 -> {
                tCalc.extendWithCharge(-3F/16F, 11F/16F, 3F/16F, charge, Easing.outQuart);
                rCalc.extendWithCharge(80, 100, 0, charge, Easing.outQuart);
            }
            case 1 -> {
                tCalc.extend(-6F / 16F, 6F / 16F, -4F / 16F, Easing.outQuart);
                rCalc.extend(-40, 100, -60, Easing.outCubic);
            }
            case 2 -> {
                tCalc.freeze();
                rCalc.freeze();
