...
public class KnifeCarveAction extends CarveAction {
    ...
    public KnifeCarveAction(Properties properties, int... duration) {
        super(properties, duration);
    }
    ...
    public void transformModelFP(int state, int frame, int duration, float charge, LivingEntity user, AnimationData data) {
        ...
        switch(state) {
            case 0 -> {
                tCalc.add(-17F/16F, 1F/16F, -2F/16F);
                rCalc.extend(55, 125, 0);
            }
            case 1 -> {
                data.tCalc.addWithCharge(0, 4F/16F, 0, Math.min(1F, (ActionTracker.get(user).getCharge() % 10F) / 3F), Easing.outSine);
                if(charge == 0F) data.tCalc.frame %= 10;
                if((charge > 0F ? ActionTracker.get(user).getCharge() : frame) % 10 > 3) {
                    if(charge == 0F) data.tCalc.frame -= 3;
                    data.tCalc.addWithCharge(0, -4F/16F, 0, Math.min(1F, ((ActionTracker.get(user).getCharge() % 10F) - 3) / 4F));
                    data.tCalc.length = 4;
                }
