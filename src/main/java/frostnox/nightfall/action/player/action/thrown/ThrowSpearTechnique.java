...
public class ThrowSpearTechnique extends ThrowTechnique {
    public ThrowSpearTechnique(DamageType[] damageType, int stunDuration, int[] duration, Properties properties, float throwVelocity, float curve, boolean spinning, @Nullable AttackEffect... effects) {
    ...
    public void transformModelFP(int state, int frame, int duration, float charge, LivingEntity user, AnimationData data) {
        ...
        switch(state) {
            case 0 -> {
                ...
                rCalc.extend(-85, 80, 0, Easing.inOutSine);
            }
            case 1 -> {
            ...
            case 2 -> {
                tCalc.extend(-6F/16F, 8F/16F, -7F/16F, Easing.outQuart);
                rCalc.extend(-105, 70, 15, Easing.outQuart);
            }
            case 3 -> {
                tCalc.freeze();
                rCalc.freeze();
