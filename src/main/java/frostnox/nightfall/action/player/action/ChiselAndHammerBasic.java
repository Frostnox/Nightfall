...
public class ChiselAndHammerBasic extends MoveSpeedPlayerAction {
    public ChiselAndHammerBasic(int[] duration, Properties properties) {
        super(properties, -0.25F, duration);
    }

    public ChiselAndHammerBasic(Properties properties, int... duration) {
        super(properties, -0.25F, duration);
    }
        ...
        if(data.size() == 6) {
            ...
            switch(state) {
                case 0 -> {
                    ...
                    leftHand.rCalc.extend(-120 + pitch, 20, 0 + pitch/4);
    ...
    public List<Component> getTooltips(ItemStack stack, @Nullable Level level, TooltipFlag isAdvanced) {
        ...
        if(ClientEngine.get().isShiftHeld() && ClientEngine.get().getPlayer() != null && PlayerData.get(ClientEngine.get().getPlayer()).hasCompletedEntry(EntriesNF.SMITHING.getId())) {
            ...
            tooltips.add(new TextComponent(" ").append(new TranslatableComponent("action.smithing.context").withStyle(ChatFormatting.GRAY))
                    .append(new TranslatableComponent("action.smithing." + AnvilAction.CUT.name().toLowerCase()).withStyle(ChatFormatting.DARK_AQUA)));
        }
        return tooltips;
    }
}
