...
public class DiagonalSwingCharged extends PlayerAttack implements IClientAction {
    public DiagonalSwingCharged(float damage, DamageType[] damageType, HurtSphere hurtSpheres, int maxTargets, int stunDuration, int[] duration, @Nullable AttackEffect... effects) {
        super(damage, damageType, hurtSpheres, maxTargets, stunDuration, duration, effects);
    }

    public DiagonalSwingCharged(float damage, DamageType[] damageType, HurtSphere hurtSpheres, int maxTargets, int stunDuration, int[] duration, Properties properties, @Nullable AttackEffect... effects) {
        super(damage, damageType, hurtSpheres, maxTargets, stunDuration, duration, properties, effects);
    }

    public DiagonalSwingCharged(DamageType[] damageType, HurtSphere hurtSpheres, int maxTargets, int stunDuration, int[] duration, Properties properties, @Nullable AttackEffect... effects) {
    ...
    public void transformModelFP(int state, int frame, int duration, float charge, LivingEntity user, AnimationData data) {
        ...
        switch(state) {
            case 0 -> {
                tCalc.extendWithCharge(-3.5F/16F, 10F/16F, 4F/16F, charge, Easing.outCubic);
                rCalc.extendWithCharge(60, 100, 0, charge, Easing.outCubic);
            }
            case 1 -> {
                tCalc.extend(-16F / 16F, -9F / 16F, -2F / 16F, Easing.outCubic);
                rCalc.extend(-60, 120, 0, Easing.outQuart);
            }
            case 2 -> {
                tCalc.freeze();
                rCalc.freeze();
    ...
    protected void transformModelSingle(int state, int frame, int duration, float charge, float pitch, LivingEntity user, EnumMap<EntityPart, AnimationData> data, AnimationCalculator mCalc) {
        ...
        if(data.size() == 6) {
            ...
            switch (state) {
                case 0 -> {
                    mCalc.extendWithCharge(0, 40, 0, charge, Easing.outSine);
                    rightHand.rCalc.extendWithCharge(-60, 15, -15, charge, Easing.outQuart);
                    rightArm.rCalc.extendWithCharge(pitch - 90, 0, 0, charge, Easing.outQuart);
                    leftArm.rCalc.extendWithCharge(pitch / 4F, 0, 0, charge, Easing.outQuart);
                    leftHand.rCalc.extendWithCharge(-45, 0, -10, charge, Easing.outQuart);
                }
                case 1 -> {
