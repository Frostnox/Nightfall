...
public abstract class BiomeSpawnGroup extends SpawnGroup {
    protected final EntityType<?> type;
    protected final int yMin, yMax, lightMin, lightMax, sizeMin, sizeRand;
    protected final RegistryObject<Biome> biome;

    public BiomeSpawnGroup(int weight, boolean friendly, EntityType<?> type, int yMin, int yMax, int lightMin, int lightMax, int sizeMin, int sizeMax, RegistryObject<Biome> biome) {
    ...
    public boolean canSpawnAt(ServerLevel level, BlockPos pos, BlockState block, int skyLight, float temperature, float humidity) {
        return pos.getY() >= yMin && pos.getY() <= yMax && skyLight >= lightMin && skyLight <= lightMax && level.getBiome(pos).is(biome.getKey());
    }
