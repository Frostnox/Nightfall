...
public class BlockEntitiesNF {
    ...
    public static final RegistryObject<BlockEntityType<ItemMoldBlockEntity>> ITEM_MOLD = BLOCK_ENTITIES.register("item_mold", () -> BlockEntityType.Builder.of(ItemMoldBlockEntity::new, Stream.concat(BlocksNF.ARMAMENT_MOLDS.values().stream().map(RegistryObject::get), Stream.of(BlocksNF.INGOT_MOLD.get(), BlocksNF.ARROWHEAD_MOLD.get(), BlocksNF.ROD_MOLD.get(), BlocksNF.BLOCK_MOLD.get(), BlocksNF.ANVIL_MOLD.get())).toArray(Block[]::new)).build(null));
