    ...
    private static class BlockLootNF extends BlockLoot {
        ...
        protected void addTables() {
            ...
            for(Metal type : BlocksNF.METAL_PLATINGS.keySet()) {
                dropOther(BlocksNF.METAL_PLATINGS.get(type).get(), ItemsNF.PLATES.get(type).get(), 4);
                dropOther(BlocksNF.METAL_PLATING_STAIRS.get(type).get(), ItemsNF.PLATES.get(type).get(), 3);
                dropSlab(BlocksNF.METAL_PLATING_SLABS.get(type).get(), ItemsNF.PLATES.get(type).get(), 2);
            ...
            dropOther(BlocksNF.MUD_BRICK_CHANNEL.get(), ItemsNF.MUD_BRICK.get(), 2);
            dropOther(BlocksNF.BRICK_CHANNEL.get(), ItemsNF.BRICK.get(), 2);
    ...
    private static class EntityLootNF extends EntityLoot {
        ...
        protected void addTables() {
            ...
            add(EntitiesNF.MERBOR_TUSKER.get(), LootTable.lootTable().withPool(pool(entry(ItemsNF.RAW_PORK.get(), 3, 4)))
                    .withPool(pool(entry(ItemsNF.MERBOR_TUSK.get(), 1, 1, List.of(PerceptionCheckFunction.with(0.5F, 0.025F)))))
                    ...
                    .withPool(pool(entry(ItemsNF.GOLDEN_MERBOR_HIDE.get(), 1F, 1F, LootItemEntityCondition.of(LootItemEntityCondition.Test.MERBOR_SPECIAL)))));
            ...
            add(EntitiesNF.ROCKWORM.get(), LootTable.lootTable().withPool(pool(entry(ItemsNF.RAW_PALE_FLESH.get(), 1, 2)))
                    .withPool(pool(entry(ItemsNF.ROCKY_SCALE.get(), 3, 4, List.of(PerceptionCountFunction.with(1, 0.5F, 0.05F))))));
            ...
            add(EntitiesNF.ECTOPLASM_LARGE.get(), LootTable.lootTable().withPool(pool(entry(ItemsNF.BONE_SHARD.get(), 2, 3)))
                    .withPool(pool(entry(ItemsNF.BONE.get(), 0, 1)))
                    .withPool(pool(entry(ItemsNF.ECTOPLASM.get(), 1, 1, List.of(PerceptionCountFunction.with(1, 0.5F, 0.05F))))));
            add(EntitiesNF.ECTOPLASM_MEDIUM.get(), LootTable.lootTable().withPool(pool(entry(ItemsNF.BONE_SHARD.get(), 1, 2)))
                    .withPool(pool(entry(ItemsNF.ECTOPLASM.get(), 1, 1, List.of(PerceptionCheckFunction.with(0.4F, 0.04F))))));
