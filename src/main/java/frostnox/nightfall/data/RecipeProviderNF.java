...
public class RecipeProviderNF extends RecipeProvider {
    ...
    protected void buildCraftingRecipes(Consumer<FinishedRecipe> consumer) {
        ...
        for(Tree type : Tree.values()) {
            ...
            BuildingRecipeBuilder.base(plank, 3, ItemsNF.PLANK_FENCE_GATES.get(type).get()).order(5).requirement(EntriesNF.ADVANCED_WOODWORKING).save(consumer);
            BuildingRecipeBuilder.base(plank, 6, ItemsNF.PLANK_DOORS.get(type).get()).order(6).requirement(EntriesNF.ADVANCED_WOODWORKING).save(consumer);
